...
module.exports = class Stale
{
  ...
  async approveIfNeeded(issue)
  {
    ...
    if (perform)
    {
      ...
      let teamMembersReq = await this.github.teams.listMembers({
          id: 1184429,
          per_page: 100 }); // might be too small someday
      const teamMembers = teamMembersReq.data.map(u => u.login)
      ...
      {
        this.logger.info("check user %s, len %d", item[2], teamMembers.filter(u => u === item[2]).length)
        this.logger.info(teamMembers.filter(u => u === item[2]))
      ...
      this.logger.info("we have %d approvals (%d from us) for %d", uniqueApprovals.length, alreadyApproved, number);
