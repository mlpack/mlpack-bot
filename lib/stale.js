...
module.exports = class Stale
{
  ...
  async autoApprove()
  {
    this.logger.info(this.config, `finding approved PRs`)

    const { owner, repo } = this.config
    const query = `repo:${owner}/${repo} is:open is:pr review:approved`
    const params = { q: query, sort: 'updated', order: 'desc',
        per_page: maxActionsPerRun }

    const results = (await this.github.search.issues(params)).data.items
    this.logger.info(this.config, `results: ${results}`)
  ...
  async approveIfNeeded(issue)
  {
    ...
    if (perform)
    {
      ...
      const approvals = reviews.data.map(review => [review.state, review.author_association, review.submitted_at]).filter(
          data => data[0].toLowerCase() === 'approved' && data[1].toLowerCase() === 'member' && new Date(data[2]) < timestamp).length

      const alreadyApproved = reviews.data.map(review => [review.state, review.user.login]).filter(data => data[0].toLowerCase() === 'approved' && data[1].toLowerCase() === 'mlpack-bot[bot]').length

      this.logger.info("we have %d approvals (%d from us) for %d", approvals, alreadyApproved, number)

      if (approvals === 1 && alreadyApproved === 0)
      {
        const prParams = { owner, repo, number, event: "APPROVE", body: approvalComment }
        this.logger.info(`would submit pr with params ${JSON.stringify(prParams)}`)
        await this.github.pullRequests.createReview(prParams)
      }
    }
  }
